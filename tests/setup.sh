...
:remove-package() {
    ...
    if [[ $mode == "cli" ]]; then
        ...
        :curl -XDELETE \
            $api_url/package/$package?path=$path\&system=archlinux\&version=$package_version
    fi
}
...
:copy-package-to-new-root() {
    ...
    if [[ $mode == "cli" ]]; then
        ...
        :curl -XPOST \
            $api_url/package/$package?path=$path\&system=archlinux\&copy-to=$new_root\&version=$package_version
    fi
}
